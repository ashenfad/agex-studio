...
**`at_tz()`** creates timezone-aware datetimes for slicing. Always pair with `local_timezone()`:
...
df = to_dataframe(events, tz=local_timezone())
...
- **tz**: Always pass `local_timezone()` (or an explicit IANA timezone).
...
- Always use `at_tz(local_timezone())` for slice bounds — never bare dates
