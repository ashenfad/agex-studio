...
_ns_mod._ViewImage.__call__ = _async_vi_call
        ...
        pyodide.globals.set("_js_test_app", (appFilesJson, actionsJson, fresh, viewportJson) => {
            return new Promise((resolve) => {
                const id = ++_testAppId;
                testAppPending.set(id, resolve);
                self.postMessage({ type: "test-app", id, appFilesJson, actionsJson, fresh, viewportJson });
            });
        });
